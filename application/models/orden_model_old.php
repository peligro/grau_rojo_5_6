   ...
   public function getResumenControlCartulinaPorLiberar()
    {
      ...
      public function getFacturasVencidasGRAUSPA($rut)
    {
        $productos = array();
        $sql_GRAUSPA = "SELECT
                    SUM (MovDebe) AS Debe,
            ...
            if (!$rs)
              {exit("Error in SQL");}
                    while (odbc_fetch_row($rs))
                    {
                      $productos[$i]["Debe"]=odbc_result($rs,"Debe");       
                      $productos[$i]["Haber"]=odbc_result($rs,"Haber");    
                      $productos[$i]["Nombre"]=odbc_result($rs,"Nombre");    
                      $productos[$i]["Referencia"]=odbc_result($rs,"Referencia");       
                      $productos[$i]["Fecha"]=odbc_result($rs,"Fecha");    
                      $i++;
                    }
            odbc_close($conn);
            if ($productos)
