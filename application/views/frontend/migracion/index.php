...
<div class="page-header"><h3>Modulo de Migración</h3></div>
    ...
    <?php   //print_r($datos);  ?>
<div>
    <a onclick="migrar();" class="btn btn-success pull-left" href="<?php echo base_url()?>migracion/migrar/">Actualizar Data</a>
    <a onclick="migrar();" class="btn btn-success pull-left" href="<?php echo base_url()?>migracion/reactualizar_migracion/">Actualizar Maestro No Tocar</a>
...
//    print_r($datos);
    ...
    { 
        ...
        if (sizeof($molde_creado)>0){
            ...
            if (($cliente->id!=$cliente_molde_creado->id) and ($cliente->id>0) and ($cliente_molde_creado->id>0)) { $error="Error en el molde, pertenece a  ".$cliente_molde_creado->razon_social.', Motivo: No son iguales, Nro de Molde <a href="'.base_url().'moldes/edit/'.$nro_molde.'">'.$nro_molde.'</a>'; $fondo="#ec5c00";$color='white';}
        }
        if ($producto_sv!='')
        ...
        <td style="background-color: <?php echo $fondo ?>;color:<?php echo $color ?>;"><?php echo $error; ?> </td>        
        ...
        <td>
            ...
            <?php 
            $rut_validado=herramientas_funciones::validaRutActaul($dato->RUT);
            if ($rut_validado)
            {
                if (($dato->RUT!='00000000-0') and ($dato->RUT!=$this->config->item('rut_repetido')) and (strstr($dato->RUT,"-")) and (!strstr($dato->RUT,"--")) and ($this->config->item('rut_repetido')!=$dato->RUT))
                {    
                    ...
                    <div id="<?php echo $dato->id."div"?>">¡Error de Rut!</div>	
                <?php }               
            }
            else { ?>
                <div id="<?php echo $dato->id."div"?>">¡Error de Rut!</div>	
