...
<?php echo form_open_multipart(null, array('class' => 'form-horizontal','name'=>'form','id'=>'form')); ?>
    ...
    <div class="control-group">
		<label class="control-label" for="usuario">Tiraje 4</label>
		<div class="controls">
			<input type="text" name="tiraje4" value="<?php echo $hoja->tiraje4?>" onkeypress="return soloNumeros(event)" />
		</div>
	</div>
    <div class="control-group">
		<label class="control-label" for="usuario">Proveedor</label>
		<div class="controls">
                    <select name="proveedor4" class="chosen-select">
                        <?php foreach ($proveedores as $value) { ?>
                        <option value="<?php echo $value->id; ?>" <?php if($value->id == $hoja->proveedor4){echo 'selected="selected"';} ?>><?php echo $value->nombre; ?></option>
                        <?php } ?>
                    </select>
		</div>
	</div>
