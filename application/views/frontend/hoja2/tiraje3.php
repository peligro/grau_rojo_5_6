...
<?php echo form_open_multipart(null, array('class' => 'form-horizontal','name'=>'form','id'=>'form')); ?>
    ...
    <div class="control-group">
		<label class="control-label" for="usuario">Tiraje 3</label>
		<div class="controls">
			<input type="text" name="tiraje3" value="<?php echo $hoja->tiraje3?>" onkeypress="return soloNumeros(event)" />
		</div>
	</div>
    <div class="control-group">
		...
		<div class="controls">
                    <select name="proveedor3" class="chosen-select">
                        ...
                        <option value="<?php echo $value->id; ?>" <?php if($value->id == $hoja->proveedor3){echo 'selected="selected"';} ?>><?php echo $value->nombre; ?></option>
                        <?php } ?>
                    </select>
		</div>
	</div>
