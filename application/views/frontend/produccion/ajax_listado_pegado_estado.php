 ...
 if($this->session->userdata('id'))
        {
            ...
            $porpagina=10;
			$datos=$this->produccion_model->getPegadoEstadoConCotizacionPaginacion($pagina,$porpagina,"limit");
			$cuantos=$this->produccion_model->getPegadoEstadoConCotizacionPaginacion($pagina,$porpagina,"cuantos");
...
//					taller/ pegadora	tiene entregas parciales?	vendedor	estado

?>
<div class="page-header"><h3>Listado de Producción Pegado Estado( <?php echo sizeof($datos)?> en total)</h3></div>
...
//fecha de ingreso	numero orde	fech lib pegado	cliente	detalle	fecha OP	cantidad de cajas	cantidad justa(s/n)	precio	tipo empaque	fcha entrega	taller/ pegadora	tiene entregas parciales?	vendedor	estado
		...
		$hayparcial=$this->produccion_model->getParcialPegado($dato->id_nodo);
    ...
    ?>
		...
		<td style="width: 6000px;"><?php echo $dato->cantidad_cajas_buenas?></td>	
		
		<td style="width: 20px;"><?php echo $dato->pegado?></td>				
